...
func BenchmarkRSAEncryption(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rsa.EncryptOAEP(md5hash, rand.Reader, pubicKey, sourceData, label)
	}
}

func BenchmarkRSADescription(b *testing.B) {
	sourceData, label, privateKey := prepareRSA()
...
func BenchmarkAESDescription(b *testing.B) {
	sourceData, nonce, gcm := prepareAES()
	encypted := gcm.Seal(nil, nonce, sourceData, nil)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		gcm.Open(nil, nonce, encypted, nil)
	}
}
